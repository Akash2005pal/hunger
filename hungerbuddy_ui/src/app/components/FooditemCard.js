...
export default function FoodItemCard({ data }) {
        ...
        <div  key={item.fooditemid} className={styles.card} onClick={()=>navigate.push(`/productdetailcomponent/${item.fooditemid}`)}>
          ...
          <div className={styles.content}>
            <h3 className={styles.name}>
              ...
              <span style={{ marginLeft: '4%' }}>
                {item.fooditemtaste == 'Spicy'
                  ? <img src={`${serverURL}/images/chilli.png`} width={16} />
                  : <></>
                }
              </span>
            </h3>
            ...
            <p className={styles.location}>
              {item.offerprice == 0
                ? <span style={{ fontWeight: 'bold', color: '#000' }}>₹{item.fullprice}</span>
                : <>
                    <span style={{ fontWeight: 'bold', marginRight: '2%', color: '#000' }}>
                      ₹{item.offerprice}
                    </span>
                    <s>₹{item.fullprice}</s>
                  </>
  ...
  return (
    <div style={{ width: '100%', marginTop: 40 }}>
      <div style={{ fontSize: 20, fontWeight: 'bold', marginBottom: 10, marginLeft: '6%' }}>
        Today's Menu
      </div>
